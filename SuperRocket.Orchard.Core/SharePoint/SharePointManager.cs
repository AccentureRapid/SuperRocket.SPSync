...
using SuperRocket.Orchard.Core.SharePoint.Adfs;
using SuperRocket.Orchard.SharePoint.Domain;
...
namespace SuperRocket.Orchard.Core.SharePoint
{
    public class SharePointManager
    {
        ...
        public static FolderView GetAllFoldersTreeFromConfig(SyncConfiguration configuration)
        {
            var rootFolderView = new FolderView();
            ...
            using (var context = spm.GetClientContext())
            {
                ...
                var list = context.Web.Lists.GetByTitle(configuration.DocumentLibrary);
                context.Load(list, 
                    p => p.RootFolder.ServerRelativeUrl, 
                    p => p.RootFolder.Name,
                    p => p.RootFolder.Files,
                    p => p.RootFolder.Files.Include(file => file.ListItemAllFields.Id),
                    p => p.RootFolder.ItemCount,
                    p => p.RootFolder.ParentFolder,
                    p => p.RootFolder.Folders
                    );
                context.ExecuteQuery();

                var rootFolder = list.RootFolder;

                rootFolderView.Files = rootFolder.Files.ToList().Select(x =>  new Orchard.SharePoint.Domain.File
                {
                    Name = x.Name,
                    Title = x.Title,
                    ServerRelativeUrl = x.ServerRelativeUrl
                }).ToList();
                rootFolderView.FilesCount = rootFolder.Files.Count;
                rootFolderView.ItemCount = rootFolder.ItemCount;
                rootFolderView.Name = rootFolder.Name;
                rootFolderView.ServerRelativeUrl = rootFolder.ServerRelativeUrl;

                rootFolderView.Children = GetAllFoldersTreeInternal(context, list.RootFolder, rootFolderView);
        ...
        private static List<FolderView> GetAllFoldersTreeInternal(ClientContext context, Folder folder, FolderView rootFolderView)
        {
            var children = new List<FolderView>();
            FolderView childFolderView = null;
            ...
            foreach (var subFolder in folder.Folders)
            {
                ...
                context.Load(subFolder,
                    p => p.Folders.Include(
                        ...
                        f => f.Name,
                        f => f.Files,
                        f => f.Files.Include(file => file.ListItemAllFields.Id),
                        f => f.ItemCount,
                        f => f.ParentFolder,
                        f => f.Folders
                        ),
                     p => p.Files,
                     p => p.Files.Include(file => file.ListItemAllFields.Id),
                     p => p.ItemCount,
                     p => p.Name,
                     p => p.ParentFolder,
                     p => p.Folders,
                ...
                childFolderView = new FolderView();

                childFolderView.Files = subFolder.Files.ToList().Select(x => new Orchard.SharePoint.Domain.File
                {
                    Name = x.Name,
                    Title = x.Title,
                    ServerRelativeUrl = x.ServerRelativeUrl
                }).ToList();
                childFolderView.FilesCount = subFolder.Files.Count;
                childFolderView.ItemCount = subFolder.ItemCount;
                childFolderView.Name = subFolder.Name;
                childFolderView.ServerRelativeUrl = subFolder.ServerRelativeUrl;

                childFolderView.Children = GetAllFoldersTreeInternal(context, subFolder, childFolderView);

                children.Add(childFolderView);
            }
