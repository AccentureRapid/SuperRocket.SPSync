...
namespace SuperRocket.Orchard.Tests
{
    public abstract class OrchardTestBase : AbpIntegratedTestBase<OrchardTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected OrchardTestBase()
        {
        ...
        protected void UsingDbContext(Action<OrchardDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<OrchardDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<OrchardDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<OrchardDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<OrchardDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<OrchardDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<OrchardDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<OrchardDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<OrchardDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<OrchardDbContext, Task<T>> func)
        {
            T result;
